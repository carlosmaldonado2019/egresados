...
if ($rol == 1) {
    $mot1=mysqli_query($con,"SELECT mot_1,mot_2,mot_3,mot_4,mot_5 FROM datos_posgrado");
}else{
    $mot1=mysqli_query($con,"SELECT mot_1,mot_2,mot_3,mot_4,mot_5 FROM datos_posgrado WHERE posgrado='$rol'");
}

while ($row=mysqli_fetch_assoc($mot1)) {
    if ($row['mot_1'] == 1) {
        $mot_1++;
    }
    if ($row['mot_1'] == 2) {
