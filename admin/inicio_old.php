	...
	<!-- topbar ends -->
		...
		<div class="row-fluid">
			...
			<?php
				include('conex.php');
				$No=0;
				$contar=mysqli_query($con,"SELECT * FROM user");
				while ($row=mysqli_fetch_row($contar)) {
					$No++;
				}
			?>
			...
			<?php
				...
				$NoA=0;
				if ($rol == 1) {
					$contar=mysqli_query($con,"SELECT * FROM datos_personales");
				}else{
					$contar=mysqli_query($con,"SELECT * FROM datos_personales a, datos_posgrado b WHERE posgrado='$rol' AND a.email=b.email");
				}
				while ($row=mysqli_fetch_row($contar)) {
					$NoA++;
				}
